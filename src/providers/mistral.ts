import { callMistralAPI, isMistralModel } from '../commands/providerClients';
...
export class MistralProvider implements ModelProvider {
	...
	public async generate(prompt: string, context: ProviderRequestContext, options?: GenerateOptions): Promise<string> {
		return callMistralAPI(buildProviderPrompt(prompt, options), context.config, !!options?.captureMetrics);
	}
